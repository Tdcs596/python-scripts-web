...
# --- SCRIPT 5 KA DESIGN (Direct HTML String) ---
INTERFACE = """
<!DOCTYPE html>
<html lang="en">
<head>
...
<script>
    async function executePing() {
        ...
        if(!target) {
            terminal.innerText = "[!] ERROR: Please enter a target IP or Domain.";
            return;
        }

        loader.style.display = "block";
        terminal.innerText = "[#] Initializing ICMP packets for: " + target;

        try {
            const response = await fetch('/run-ping', {
                method: 'POST',
...
@app.route('/')
def home():
    # Direct HTML return kar raha hai bina kisi folder ke
...
def run_ping():
    ...
    try:
        # Render (Linux) command setup
        # -c 4 = 4 packets, -W 2 = 2 second timeout
...
if __name__ == '__main__':
    # Render automatic PORT handle karta hai
